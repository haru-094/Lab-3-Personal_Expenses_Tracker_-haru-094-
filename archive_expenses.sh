...
# searching
    ...
    read -p "Please enter the date that you want to search for: " user_input_date
    if [[ ! $user_input_date =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
        echo "This date is invaild!"
        echo "must be in format of YYYY-mm-dd"
        exit 1
    fi

    if [ -d "$archives/$user_input_date" ]; then
        for search_file in expenses-*.txt; do
